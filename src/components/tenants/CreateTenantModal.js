...
export default function CreateTenantModal({ isOpen, onClose, onSuccess }) {
    ...
    const onSubmit = useCallback(async (data) => {
        ...
        try {
            ...
            const errorMessage = err.response?.data?.message || "Erreur lors de la création du restaurant";
    ...
    return (
        <Dialog open={isOpen} onOpenChange={handleClose}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>
                        {createdTenant ? "Restaurant créé avec succès" : "Créer un nouveau restaurant"}
                    </DialogTitle>
                    <DialogDescription>
                        {createdTenant
                ...
                {createdTenant ? (
                    <div className="space-y-4">
                        <div className="bg-green-50 border border-green-200 rounded-md p-4">
                            <h3 className="font-medium text-green-800 mb-2">Restaurant créé</h3>
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>Code du restaurant *</FormLabel>
                                        <FormControl>
                                            <Input placeholder="RESTAURANT_1" {...field} />
                                        </FormControl>
                                        <FormDescription>
                                            Code unique utilisé pour identifier le restaurant (ex: RESTAURANT_1)
                                        </FormDescription>
                                        <FormMessage />
                                    </FormItem>
                                )}
